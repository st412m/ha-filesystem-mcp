...
`sqlite_query` accepts one statement starting with `SELECT`, `WITH` or `VALUES`. The `EXPLAIN` keyword passes the statement check, but the query is wrapped as `SELECT * FROM (<sql>)` and SQLite rejects that construction with a syntax error, so `EXPLAIN` cannot be run through `sqlite_query`. The multiple-statement check is a plain search for `;`, so a semicolon inside a string literal is refused with `MULTIPLE_STATEMENTS` as well; rewrite the query without it (for example with `char(59)`), or split it.
